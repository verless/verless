...
* **`Type`** _(String)_: The page's content type which automatically defines the template to be used. If the content type is `blog`, a template called `blog.html` will be used for rendering. In future verless versions, content types may provide more configuration other than the template to use.
